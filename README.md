...
| 2 | **手机状态·红色** 内容同上 |
| 3 | **照片**（PH1，3926页） |
| 4 | **时钟**（CLK_BG背景+ASC64 ASCII字库，默认`时:分`，可切换`时:分:秒`/`时:分:秒.毫秒`） |
...
注意：
 ...
 - 默认固件只内置了24×33数字图模，因此时间显秒/毫秒首次切换默认黑色背景，加载约1分钟以显示背景图片，后续可直接读取缓存不用加载
 - 若要避免每次切到投屏时都出现授权弹窗，ADB执行
```
appops set com.msu2.android PROJECT_MEDIA allow
```
